...
double *normalize_gt_2d(double *gt_2d, Model t_model) 
{
	double *t_gt = (double *)malloc(sizeof(double)* JointNum * 2); //!!! malloc is vital
	for (int i = 0; i < JointNum; i++)
	{
#ifdef Global
		for (int i = 0; i < JointNum; i++)
		{
			t_gt[i * 2] = (gt_2d[i * 2] * t_model.crop_size - t_model.bbx_x1) / t_model.crop_size;
			t_gt[i * 2 + 1] = (gt_2d[i * 2 + 1] * t_model.crop_size - t_model.bbx_y1) / t_model.crop_size;
		}
#else
