...
def vis_mapping_cylinder(cylinder, kps_lines,):
    fig = plt.figure()
    ...
    for z in range(0, depth_dim):
        for y in range(0, output_shape):
            for x in range(0, output_shape):
                cc = cylinder[z][y][x].item()#data.cpu().numpy()
                if cc > 0.7:
    ...
    def load_data(self):
        ...
        for content_id in range(self.lbd_contents, self.ubd_contents):
            ...
            for j in range(self.joint_num):
                ...
                #joint_depth[j] = self.jt_3d[content_id * self.joint_num + j][2] - root_z
...
def load_model():
    ...
    #ckpt.pop('module.head.ske_3d_layer.weight', None)
    #ckpt.pop('module.head.ske_3d_layer.bias', None)
    ...
    #ckpt_2dske = torch.load('snapshot_16.pth.tar')
    #ckpt_2dske = torch.load('resnet50/model256_mpii_ist_' + model_str + '.pth')
    
    #ckpt_2dske = ckpt_2dske['network']
...
def train(epoch):
    ...
    for iter_id in range(1):
        ...
        for batch_idx, (style, content, joint_img, joint_vis,  joints_have_depth, gt_ske, data_source, gt_3dske, joint_bone, neg_thorax) in enumerate(train_loader):
            ...
            #joint_depth = joint_depth.cuda()
            #joint_depth = joint_depth.type(torch.cuda.FloatTensor)
            
            norm_bone = joint_bone.cuda()
            neg_thorax = neg_thorax.cuda()
            #opt = Variable(content.data.clone(), requires_grad=True)
            #print(opt)
            ...
            #print('neg_thorax pred ', neg_thorax_pred)
            #print('norm_bone pred', norm_bone_pred)
            ...
            #print('unnorm neg_thorax gt ', neg_thorax)
            #print('unnorm_bone gt', norm_bone)
            ...
            #print('gt reconst joint ', joint_gt_reconstruct)
            ...
            #print('recons abs diff ', reconstruct_diff)
            ...
            for t in range(batch_size):
                ...
                #cv2.imshow("cur_ske", cur_ske_img)
                #cv2.waitKey(0)
            ...
            #opt_self_sup = model_self_sup_deconv(style, bone_img_arr, bone_img_arr_per_bone)
            ...
            #print(layer_losses)
            #ll_self_sup = layer_losses_self_sup
            ...
            #ll_self_sup_dis_loss = nn.SmoothL1Loss()
            #ll_self_sup_dis_loss = ll_self_sup_dis_loss(opt_self_sup, opt)

            #ll_self_sup = nn.SmoothL1Loss()
            #ll_self_sup = ll_self_sup(opt_self_sup, opt)
            #print('ll_self_sup ', ll_self_sup)
            ...
            # ==== CONTENT ENT SELF SUP
            #content_self_sup_ent_loss = 0
            #for i in range(len(content_targets_out_opt)):
            #    b, c, h, w = content_targets_out_opt[i].shape
            #    #GT content VGG features (content)
            #    content_targets_out_opt[i] = content_targets_out_opt[i].reshape(b, c, h * w)
            #    content_targets_out_opt[i] = nn.Softmax(dim=2)(content_targets_out_opt[i])
            #    content_targets_out_opt[i] = content_targets_out_opt[i].reshape(b, c, h, w)

            #    #opt image VGG features(content)
            #    opt_self_sup_content_out[i] = opt_self_sup_content_out[i].reshape(b, c, h * w)
            #    opt_self_sup_content_out[i] = nn.Softmax(dim=2)(opt_self_sup_content_out[i])
            #    opt_self_sup_content_out[i] = opt_self_sup_content_out[i].reshape(b, c, h, w)
            #    cur_loss = js_div_loss_2d(content_targets_out_opt[i], opt_self_sup_content_out[i])
            #    #print(i, 'self sup content JS', cur_loss)
            #    content_self_sup_ent_loss += cur_loss

            # ==== STYLE ENT SELF SUP LOSS
            # ==== STYLE
            #style_self_sup_ent_loss = 0
            #for i in range(len(style_targets_out_opt)):
            #    b, c, h, w = style_targets_out_opt[i].shape
                #GT style VGG features (style)
            #    style_targets_out_opt[i] = style_targets_out_opt[i].reshape(b, c, h * w)
            #    style_targets_out_opt[i] = nn.Softmax(dim=2)(style_targets_out_opt[i])
            #    style_targets_out_opt[i] = style_targets_out_opt[i].reshape(b, c, h, w)

                #opt image VGG features(style)
            #    opt_self_sup_style_out[i] = opt_self_sup_style_out[i].reshape(b, c, h * w)
            #    opt_self_sup_style_out[i] = nn.Softmax(dim=2)(opt_self_sup_style_out[i])
            #    opt_self_sup_style_out[i] = opt_self_sup_style_out[i].reshape(b, c, h, w)
            #    cur_loss = js_div_loss_2d(style_targets_out_opt[i], opt_self_sup_style_out[i])
                #print(i, 'style JS', cur_loss)
            #    style_self_sup_ent_loss += cur_loss
                ...
                if cnt % show_cnt == 0:
                    ...
                    if vis_2d:
                        ...
                        if vis_3d:
                            ...
                            #f = c = 1000
                            #filename = '../datasets/testset_vis3d/' + str(cnt) + '.png'
                            #pre_3d_kpt[:,0], pre_3d_kpt[:,1], pre_3d_kpt[:,2] = pixel2cam(pre_2d_kpt, f, c)
                    ...
                    #    vis_mapping_cylinder(gt_mapping_cylinder[b], train_dataset.skeleton[0])
            
                break

            cnt += 1 #Add one iteration
            
            #mapping_cylinder_layer_loss = nn.MSELoss()
            #mapping_cylinder_layer_loss = DataParallelCriterion(mapping_cylinder_layer_loss)
            #mapping_cylinder_layer_loss = mapping_cylinder_layer_loss(mapping_cylinder, gt_mapping_cylinder)
            
            #ske_3d_layer_loss = nn.SmoothL1Loss()
            #ske_3d_layer_loss = DataParallelCriterion(ske_3d_layer_loss)
            #ske_3d_layer_loss = ske_3d_layer_loss(pred_3dske, gt_3dske)
            ...
            #print(joint_depth)
            #print(depth_reg_2dske_to_pose)
            #jnt_loss_2dske = depth_reg_loss
            #jnt_loss_2dske = JointLocationLoss()
            #jnt_loss_2dske = jnt_loss_2dske(heatmap_out_2dske_to_pose, joint_img, joint_vis, joints_have_depth, count_only_depth = True)
            #coord_out_2dske = soft_argmax(heatmap_out_2dske_to_pose, global_joint_num)
            ...
            #print('entropy (style) self sup :  ', style_self_sup_ent_loss)
            #print('entropy (content) self sup: ', content_self_sup_ent_loss)
            ...
            #print('jnt loss: (from 2dske)      ', jnt_loss_2dske)
            #print('mapping cylinder loss: ', mapping_cylinder_layer_loss)
            #print('3d ske loss :', ske_3d_layer_loss)
            ...
            print('proj loss:                  ', ske_map_proj_loss)
